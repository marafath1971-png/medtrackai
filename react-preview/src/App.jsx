...
/* ══════════════════════════════════════════════
   DESIGN TOKENS
══════════════════════════════════════════════ */
// Olive-inspired Light/Earthy Onboarding Theme
const D = {
  bg: "#F5F5F0", card: "#FFFFFF", border: "#E2E8E4", text: "#1A2621", sub: "#64736D",
  lime: "#3A7D6A", limeDark: "#2D5E4E", limeDim: "rgba(58,125,106,0.12)",
  green: "#3A7D6A", greenLight: "#D1FAE5",
...
const MascotSVG = ({size=62, mood="content"}) => (
  <svg width={size} height={size*1.25} viewBox="0 0 70 90" role="img">
    <rect x="15" y="6" width="40" height="78" rx="20" fill={mood==="sleepy"?"#5A6168":"#4A9E85"}/>
    <rect x="15" y="42" width="40" height="6" fill={mood==="sleepy"?"#474D52":"#3A7D6A"}/>
    {mood==="energetic" && (
      <>
        <circle cx="13" cy="14" r="1.6" fill="#7CF5CF"/>
        <circle cx="58" cy="20" r="1.6" fill="#7CF5CF"/>
        <circle cx="11" cy="60" r="1.6" fill="#7CF5CF"/>
        <line x1="21" y1="86" x2="28" y2="83" stroke="#3A4A44" strokeWidth="2" strokeLinecap="round"/>
        <line x1="49" y1="86" x2="42" y2="83" stroke="#3A4A44" strokeWidth="2" strokeLinecap="round"/>
      </>
    )}
    {mood==="sleepy" ? (
      <g transform="rotate(-8 35 45)">
        <path d="M24 27 Q28 30 32 27" stroke="#0A0A0C" strokeWidth="3" fill="none" strokeLinecap="round"/>
        <path d="M38 27 Q42 30 46 27" stroke="#0A0A0C" strokeWidth="3" fill="none" strokeLinecap="round"/>
        <line x1="31" y1="37" x2="39" y2="37" stroke="#0A0A0C" strokeWidth="3" strokeLinecap="round"/>
        <text x="52" y="14" fontSize="9" fill="#7B828A" fontWeight="500">z</text>
        <text x="58" y="8" fontSize="11" fill="#7B828A" fontWeight="500">z</text>
      </g>
    ) : (
      <>
        <circle cx="28" cy="26" r="4" fill="#0A0A0C"/>
        <circle cx="42" cy="26" r="4" fill="#0A0A0C"/>
        <path d={mood==="energetic"?"M26 33 Q35 43 44 33":"M28 35 Q35 39 42 35"} stroke="#0A0A0C" strokeWidth="3" fill="none" strokeLinecap="round"/>
      </>
    )}
  </svg>
);
...
const OB_STEPS = [
  ...
  // 21 - Commit
  { type:"commit" },
  // 22 - Paywall (internally manages 3 sub-steps)
  { type:"paywall" },
];
  ...
  return (
    <div style={{fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif", background:D.bg, minHeight:"100vh", maxWidth:430, margin:"0 auto", display:"flex", flexDirection:"column", position:"relative", overflow:"hidden"}}>
      ...
      <div key={animKey} className={step>0?(dir>=0?"ob-animate":"ob-animate-left"):""} style={{flex:1,display:"flex",flexDirection:"column",position:"relative",zIndex:1,paddingTop:showProgress?72:0}}>
        ...
        {s.type==="commit"  && <CommitStep onNext={()=>goNext()}/>}
        {s.type==="paywall" && (
          <PaywallStep
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 32px",textAlign:"center",minHeight:"100vh"}}>
      <div className="s1" style={{marginBottom:24}}>
        <div style={{width:100,height:100,background:D.limeDim,borderRadius:32,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px",animation:"floatUp 3s ease-in-out infinite"}}>
          <MascotSVG size={60} mood="energetic" />
        </div>
        <h1 style={{fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",fontSize:36,fontWeight:800,color:D.text,margin:"0 0 12px",letterSpacing:"-1.5px",lineHeight:1.1}}>
          MedTrack<span style={{color:D.lime}}>AI</span>
        </h1>
        <p style={{color:D.sub,fontSize:16,lineHeight:1.5,margin:0,fontWeight:500}}>Feel better fast. The easiest way to start tracking your health journey.</p>
      </div>
      ...
      <div className="s3" style={{width:"100%"}}>
        <button onClick={onNext} style={{width:"100%",padding:"18px",background:D.lime,border:"none",borderRadius:16,fontSize:17,fontWeight:800,color:"#FFFFFF",cursor:"pointer",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",animation:"glowPulse 2s ease-in-out infinite"}}>
          Get Started
        </button>
...
function TextStep({s,form,onChange,onNext}) {
  ...
  return(
    <div style={{flex:1,padding:"20px 24px 40px",display:"flex",flexDirection:"column"}}>
      ...
      />
      <button onClick={onNext} disabled={!val.trim()} style={{width:"100%",padding:"17px",background:val.trim()?D.lime:D.border,border:"none",borderRadius:16,fontSize:17,fontWeight:800,color:val.trim()?"#FFFFFF":D.sub,cursor:val.trim()?"pointer":"default",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",marginTop:24,transition:"all 0.2s"}}>
        {val.trim()?`Continue`:"Enter your answer"}
      </button>
    </div>
  );
}
        ...
        {s.opts.map((opt,i)=>{
          ...
          return(
            <button key={i} onClick={()=>onSelect(opt.v)}
              style={{display:"flex",alignItems:"center",gap:grid?10:12,padding:grid?"14px 12px":"14px 18px",background:sel?D.limeDim:D.card,border:sel?`1px solid ${D.lime}`:`1px solid ${D.border}`,borderRadius:16,cursor:"pointer",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",transition:"all 0.15s",textAlign:"left",flexDirection:grid?"column":"row"}}>
              <span style={{fontSize:grid?28:24,lineHeight:1}}>{opt.e}</span>
              <span style={{fontSize:14,fontWeight:600,color:sel?D.limeDark:D.text,lineHeight:1.3,flex:grid?undefined:1}}>{opt.v}</span>
              {!grid&&sel&&<div style={{width:20,height:20,borderRadius:99,background:D.lime,display:"flex",alignItems:"center",justifyContent:"center",marginLeft:"auto",flexShrink:0}}><Ic d={ic.check} size={11} c="#FFFFFF" w={2.5}/></div>}
            </button>
          );
        })}
      </div>
      <button onClick={onNext} disabled={!val} style={{width:"100%",padding:"17px",background:val?D.lime:D.border,border:"none",borderRadius:16,fontSize:17,fontWeight:800,color:val?"#FFFFFF":D.sub,cursor:val?"pointer":"default",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",marginTop:16,transition:"all 0.2s"}}>
        ...
        {s.opts.map((opt,i)=>{
          ...
          return(
            <button key={i} onClick={()=>onToggle(opt.v)}
              style={{display:"flex",alignItems:"center",gap:12,padding:"14px 18px",background:sel?D.limeDim:D.card,border:sel?`1px solid ${D.lime}`:`1px solid ${D.border}`,borderRadius:16,cursor:"pointer",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",transition:"all 0.15s",textAlign:"left",boxShadow:!sel?"0 2px 8px rgba(0,0,0,0.02)":"none"}}>
              <span style={{fontSize:22,lineHeight:1}}>{opt.e}</span>
              <span style={{fontSize:14,fontWeight:600,color:sel?D.limeDark:D.text,flex:1}}>{opt.v}</span>
              {sel&&<div style={{width:20,height:20,borderRadius:99,background:D.lime,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Ic d={ic.check} size={11} c="#FFFFFF" w={2.5}/></div>}
            </button>
          );
        })}
      </div>
      <button onClick={onNext} disabled={vals.length===0} style={{width:"100%",padding:"17px",background:vals.length>0?D.lime:D.border,border:"none",borderRadius:16,fontSize:17,fontWeight:800,color:vals.length>0?"#FFFFFF":D.sub,cursor:vals.length>0?"pointer":"default",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",marginTop:16,transition:"all 0.2s"}}>
        {vals.length>0?`Continue`:"Select at least one"}
      </button>
    </div>
  );
}
...
function TimeStep({s,form,onChange,onNext}) {
  ...
  return(
    <div style={{flex:1,padding:"20px 24px 40px",display:"flex",flexDirection:"column"}}>
      ...
      <button onClick={onNext} style={{width:"100%",padding:"17px",background:D.lime,border:"none",borderRadius:16,fontSize:17,fontWeight:800,color:"#FFFFFF",cursor:"pointer",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",marginTop:16}}>
        Set {fmt(v.h,v.m)} →
      </button>
    </div>
  );
  ...
  return(
    <div style={{flex:1,padding:"20px 24px 40px",display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center"}}>
      ...
      <div className="s3" style={{width:"100%",display:"flex",flexDirection:"column",gap:10}}>
        <button onClick={onAllow} style={{width:"100%",padding:"17px",background:D.lime,border:"none",borderRadius:16,fontSize:17,fontWeight:800,color:"#FFFFFF",cursor:"pointer",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",animation:"glowPulse 2s ease-in-out infinite"}}>
          Allow Notifications 🔔
        </button>
...
function PlanReadyStep({form,onNext}) {
  ...
  return (
    <div style={{flex:1,padding:"20px 24px 40px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center"}}>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:20, width:"100%"}}>
        <div style={{fontSize:80, fontWeight:800, color:D.text, fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif", letterSpacing:"-2px"}}>{pct}%</div>
        <h2 style={{fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",fontSize:24,fontWeight:700,color:D.text,margin:"0 0 8px",letterSpacing:"-0.5px",lineHeight:1.2, height:60}}>
          {pct < 30 ? "Analyzing responses..." : pct < 70 ? "Building your health profile..." : pct < 100 ? "Activating safeguards..." : "We're setting everything up for you"}
        </h2>
        <div style={{height:6,background:D.border,borderRadius:99,overflow:"hidden", width:"100%", maxWidth:240, margin:"10px auto"}}>
          <div style={{height:"100%",background:D.lime,borderRadius:99,transition:"width 0.1s ease",width:`${pct}%`}}/>
        </div>
        {pct === 100 && (
          <div className="s1" style={{marginTop:20, width:"100%", textAlign:"left"}}>
            {[
              "Personalized reminder schedule",
              "Symptom tracking setup",
              "AI adherence optimization"
            ].map((task, i) => (
              <div key={i} style={{display:"flex", alignItems:"center", gap:10, marginBottom:10}}>
                <div style={{width:20,height:20,borderRadius:99,background:D.lime,display:"flex",alignItems:"center",justifyContent:"center"}}><Ic d={ic.check} size={11} c="#FFFFFF" w={2.5}/></div>
                <span style={{fontSize:14, fontWeight:600, color:D.text}}>{task}</span>
              </div>
            ))}
          </div>
        )}
      </div>
      <button onClick={onNext} disabled={pct<100} style={{width:"100%",padding:"18px",background:pct===100?D.lime:D.border,border:"none",borderRadius:16,fontSize:17,fontWeight:800,color:pct===100?"#FFFFFF":D.sub,cursor:pct===100?"pointer":"default",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",marginTop:24,transition:"all 0.2s"}}>
        {pct===100?"See My Plan":"Please wait..."}
      </button>
    </div>
  );
}
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",background:D.bg}}>
        ...
        {PLANS.map(p=>{
          ...
          return(
            <button key={p.id} onClick={()=>setSelectedPlan(p.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:14,padding:"16px 18px",background:sel?D.limeDim:D.card,border:`2px solid ${sel?D.lime:D.border}`,borderRadius:16,cursor:"pointer",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",textAlign:"left",transition:"all 0.2s",marginBottom:10,position:"relative"}}>
              {p.badge&&sel&&<div style={{position:"absolute",top:-10,right:14,background:D.lime,color:"#FFFFFF",fontSize:10,fontWeight:800,padding:"3px 10px",borderRadius:99}}>{p.badge}</div>}
      ...
      <div style={{padding:"0 20px 32px"}}>
        <button onClick={onNext} style={{width:"100%",padding:"17px",background:D.lime,border:"none",borderRadius:16,fontSize:17,fontWeight:800,color:"#FFFFFF",cursor:"pointer",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",animation:"glowPulse 2s ease-in-out infinite"}}>
          Start Free Trial →
        </button>
...
function PaywallStep2({form,selectedPlan,promoSuccess,onNext,onBack}) {
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",padding:"24px 20px 32px",background:D.bg}}>
      ...
      <button onClick={onNext} style={{width:"100%",padding:"17px",background:D.lime,border:"none",borderRadius:16,fontSize:17,fontWeight:800,color:"#FFFFFF",cursor:"pointer",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif"}}>
...
function PaywallStep3({form,selectedPlan,promoSuccess,onDone,onBack}) {
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",padding:"24px 20px 32px",background:D.bg}}>
      ...
      <button onClick={onDone} style={{width:"100%",padding:"18px",background:D.lime,border:"none",borderRadius:16,fontSize:17,fontWeight:800,color:"#FFFFFF",cursor:"pointer",fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",animation:"glowPulse 2s ease-in-out infinite"}}>
...
function CommitStep({onNext}) {
  const [held, setHeld] = useState(0);
  const [isHolding, setIsHolding] = useState(false);

  useEffect(() => {
    let interval;
    if (isHolding && held < 100) {
    ...
    } else if (!isHolding && held < 100) {
      setHeld(0);
    }
    return () => clearInterval(interval);
  }, [isHolding, held, onNext]);
  ...
  return (
    <div style={{flex:1,padding:"40px 24px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center"}}>
      <h2 style={{fontFamily:"'Figtree',-apple-system,'Helvetica Neue',sans-serif",fontSize:28,fontWeight:800,color:D.text,marginBottom:16,letterSpacing:"-1px",lineHeight:1.2}}>
        Commit to Your Health
      </h2>
      <p style={{color:D.sub,fontSize:16,marginBottom:60,fontWeight:500}}>Hold the button below to confirm you're ready to start building better habits.</p>
      ...
      <div 
        ...
        style={{
          ...
          background:held===100?D.lime:D.card, 
          border:`4px solid ${held===100?D.lime:D.border}`,
          display:"flex", alignItems:"center", justifyContent:"center",
          cursor:"pointer", position:"relative", overflow:"hidden",
          boxShadow:held>0?`0 0 40px ${D.limeDim}`:"0 10px 30px rgba(0,0,0,0.05)",
      ...
      >
        <div style={{
          ...
          height:`${held}%`, background:D.limeDim, 
          transition:"height 0.1s linear", zIndex:1
        }}/>
        <div style={{zIndex:2, display:"flex", flexDirection:"column", alignItems:"center"}}>
          <MascotSVG size={80} mood={held===100?"energetic":(isHolding?"energetic":"content")}/>
        </div>
      </div>
      <p style={{marginTop:32, color:held>0?D.lime:D.sub, fontWeight:700, fontSize:15, transition:"color 0.2s"}}>
